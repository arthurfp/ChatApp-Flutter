...
class PinInputTextField extends StatefulWidget {
  ...
  PinInputTextField({
    ...
    this.onChanged,
        ...
        assert(decoration.hintText == null || decoration.hintText.length == pinLength),
        assert(decoration is BoxTightDecoration ||
            (decoration is UnderlineDecoration && pinLength - 1 == (decoration.gapSpaces?.length ?? (pinLength - 1))) ||
            (decoration is BoxLooseDecoration && pinLength - 1 == (decoration.gapSpaces?.length ?? (pinLength - 1)))),
        inputFormatters = inputFormatter == null
            ? <TextInputFormatter>[FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(pinLength)]
            : inputFormatter
...
//          offset: _effectiveController.text.runes.length);
  ...
  void _updateText() {
    if (_effectiveController.text.runes.length > widget.pinLength) {
      _text = String.fromCharCodes(_effectiveController.text.runes.take(widget.pinLength));
    } else {
  ...
  void didUpdateWidget(PinInputTextField oldWidget) {
    ...
    if (oldWidget.pinLength > widget.pinLength && _text.runes.length > widget.pinLength) {
      setState(() {
        _text = _text.substring(0, widget.pinLength);
        _effectiveController.text = _text;
        _effectiveController.selection = TextSelection.collapsed(offset: _text.runes.length);
      });
    }
  }
...
class _PinPaint extends CustomPainter {
  ...
  _PinPaint({
    ...
    this.themeData,
          ...
          errorTextStyle:
              decoration.errorTextStyle ?? themeData.textTheme.caption.copyWith(color: themeData.errorColor),
          hintTextStyle: decoration.hintTextStyle ?? themeData.textTheme.headline2.copyWith(color: themeData.hintColor),
        );

  @override
  bool shouldRepaint(CustomPainter oldDelegate) => !(oldDelegate is _PinPaint && oldDelegate.text == this.text);
  ...
  _drawBoxTight(Canvas canvas, Size size) {
    ...
    double singleWidth = (size.width - dr.strokeWidth * (pinLength + 1)) / pinLength;

    for (int i = 1; i < pinLength; i++) {
      double offsetX = singleWidth + dr.strokeWidth * i + dr.strokeWidth / 2 + singleWidth * (i - 1);
      canvas.drawLine(Offset(offsetX, dr.strokeWidth), Offset(offsetX, mainHeight - dr.strokeWidth), borderPaint);
    }
    ...
    bool obscureOn = decoration.obscureStyle != null && decoration.obscureStyle.isTextObscure;
    ...
    text.runes.forEach((rune) {
      ...
      startX = dr.strokeWidth * (index + 1) + singleWidth * index + singleWidth / 2 - textPainter.width / 2;
  ...
  _drawBoxLoose(Canvas canvas, Size size) {
    ...
    double gapTotalLength = dr.gapSpaces?.reduce((a, b) => a + b) ?? (pinLength - 1) * dr.gapSpace;

    List<double> gapSpaces = dr.gapSpaces ?? List.filled(pinLength - 1, dr.gapSpace);

    /// Calculate the width of each underline.
    double singleWidth = (size.width - dr.strokeWidth * 2 * pinLength - gapTotalLength) / pinLength;
    ...
    for (int i = 0; i < pinLength; i++) {
      ...
      } else if (decoration.errorText != null && decoration.errorText.isNotEmpty) {
      ...
      startX += singleWidth + dr.strokeWidth * 2 + (i == pinLength - 1 ? 0 : gapSpaces[i]);
    }
  ...
  _drawUnderLine(Canvas canvas, Size size) {
    ...
    text.runes.forEach((rune) {
      ...
      startX = singleWidth * index + singleWidth / 2 - textPainter.width / 2 + _sumList(gapSpaces.take(index));
      textPainter.paint(canvas, Offset(startX, startY));
      index++;
    });
...
class _PinInputTextFormFieldState extends FormFieldState<String> {
  ...
  void didUpdateWidget(PinInputTextFormField oldWidget) {
    ...
    /// If the newLength is shorter than now and the current text length longer
    /// than [pinLength], So we should cut the superfluous subString.
    if (oldWidget.pinLength > widget.pinLength && value.runes.length > widget.pinLength) {
      setState(() {
